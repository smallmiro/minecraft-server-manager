...
### Platform Configuration Upgrade

When updating mcctl to a new version, the bundled `.env.example` template may contain new environment variables. Use `mcctl upgrade` to detect and apply these changes:

```bash
# Preview what would change
mcctl upgrade --dry-run

# Apply changes interactively (prompts for new variable values)
mcctl upgrade

# Apply with default values (for scripts/CI)
mcctl upgrade --non-interactive
```

**How it works:**
1. Compares the bundled `.env.example` template with your existing `.env`
2. New active variables (e.g., `NEW_VAR=default`) are added with prompted or default values
3. New commented variables (e.g., `# OPTIONAL_VAR=value`) are added as comments
4. Existing values are **never** modified (append-only)
5. Updates `servers/_template/` files (docker-compose.yml, config.env) to latest templates
6. Records `templateVersion` in `.mcctl.json` for tracking

**Variables affected by upgrade:**
- Platform-level `.env` variables (HOST_IP, network settings, backup config, etc.)
- NOT server-level `config.env` variables (those are per-server and manually managed)

**Example upgrade output:**
```
mcctl upgrade (v2.4.0)
Current template version: 2.3.0
CLI version: 2.4.0

Found 2 new variable(s) in template:
  + PLAYIT_SECRET_KEY=your-secret-key-here (External Access)
  + AUDIT_AUTO_CLEANUP=true (Audit Logs)

Enter values for new variables (press Enter for default):
  PLAYIT_SECRET_KEY: [your-secret-key-here]
  AUDIT_AUTO_CLEANUP: [true]

.env updated with 2 new variable(s).
Updated 2 template file(s) in servers/_template.
Upgrade complete: .env updated with new variables, 2 template file(s) updated, templateVersion set to 2.4.0.
```
...
### playit.gg External Access

| Variable | Location | Description |
|----------|----------|-------------|
| `PLAYIT_SECRET_KEY` | Platform `.env` | Secret key from playit.gg agent creation |
...
# Register domain for a server
mcctl playit domain <server> <playit-domain>
```

**Docker Configuration (auto-generated by mcctl):**
```yaml
services:
  playit:
...
**Note:** The playit-agent uses `network_mode: host` to access `localhost:25565` (mc-router). The `profiles: [playit]` setting means it only starts when explicitly requested via `mcctl playit start`.
...
### Server with playit.gg External Access

```env
# config.env (same as any server - playit is platform-level)
TYPE=PAPER
VERSION=1.21.1
MEMORY=4G
USE_AIKAR_FLAGS=true
...
EXTRA_ARGS=--universe /worlds/
ENABLE_RCON=true
MOTD=Public Server via playit.gg
```

```bash
# Step 1: One-time playit.gg setup
mcctl playit setup
# Enter SECRET_KEY from https://playit.gg/account/agents/new-docker

# Step 2: Start playit agent
mcctl playit start

# Step 3: Create tunnel at playit.gg website
# Go to https://playit.gg/account/tunnels
# Add tunnel: Minecraft Java → localhost:25565
# Get address: xx-xx.craft.playit.gg

# Step 4: Create server with playit domain
mcctl create public-srv -t PAPER -v 1.21.1
mcctl playit domain public-srv xx-xx.craft.playit.gg

# Step 5: Share address with friends
# They connect to: xx-xx.craft.playit.gg
```

> **Tip**: Each server needs its own playit.gg tunnel. Create multiple tunnels for multiple servers and register each with `mcctl playit domain`.
...
### playit.gg Connection Issues

**Symptoms:**
- External players can't connect via playit.gg domain
- playit-agent container not running
- Wrong server reached
...
# Look for playit domain in hostname labels
```
...
| Issue | Fix |
|-------|-----|
| Agent not running | `mcctl playit start` |
| Invalid SECRET_KEY | `mcctl playit setup` (re-enter key) |
| No tunnel created | Create at https://playit.gg/account/tunnels |
| Domain not registered | `mcctl playit domain <server> <domain>` |
| mc-router not running | `mcctl up` or `mcctl router start` |
