...
/**
 * BFF Proxy for /api/proxy/servers/[name]
...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const response = await fetch(`${API_URL}/v1/servers/${encodeURIComponent(name)}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { code: 'API_ERROR', message: error.message || 'Failed to fetch server' },
