...
/**
 * BFF Proxy for /api/proxy/servers/[name]/restart
...
export async function POST(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const response = await fetch(
      `${API_URL}/v1/servers/${encodeURIComponent(name)}/restart`,
      {
        method: 'POST',
        headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { code: 'API_ERROR', message: error.message || 'Failed to restart server' },
