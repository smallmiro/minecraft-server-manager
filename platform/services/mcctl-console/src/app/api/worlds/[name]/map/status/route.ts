 ...
 * Proxy to mcctl-api: whether a BlueMap web map exists and its dimensions (#529)
 */
export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const data = await client.getWorldMapStatus(name);
    ...
    return NextResponse.json({ error: 'InternalServerError', message: 'Failed to fetch map status' }, { status: 500 });
  }
}
