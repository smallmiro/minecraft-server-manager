 ...
 * Proxy to mcctl-api: parsed level.dat metadata + on-disk structure (#525)
 */
export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const data = await client.getWorldInfo(name);
    ...
    return NextResponse.json({ error: 'InternalServerError', message: 'Failed to fetch world info' }, { status: 500 });
  }
}
